...
from guided_diffusion_dpmsolver import NoiseScheduleVP,model_wrapper,DPM_Solver
import enum
import math
...
class GaussianDiffusion:
    ...
    def add_onestep_noise(self, x_t, t):
        ...
        betas_tensor = torch.from_numpy(self.betas).to(device)
        x_out = torch.sqrt(1. - betas_tensor[t[0]]) * x_t + torch.sqrt(betas_tensor[t[0]]) * noise
    ...
    def p_mean_variance(
        ...
        # print("expected:", (B, C * 2, *x.shape[2:]))
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            pred_xstart = process_xstart(
                self._predict_xstart_from_xprev(x_t=x, t=t, xprev=noise_pred) # prevoiusly was xprev=model_output
            )
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=x, t=t, eps=noise_pred) # prevoiusly was eps=model_output
                )
    ...
    def p_sample( 
        ...
        if inpa_inj_sched_prev: 
            if "ref_img" in model_mask_kwargs: 
    ...
    def p_sample_loop_progressive( 
        ...
        if model_kwargs is None: 
            model_kwargs = {} 
        if model_mask_kwargs is None: 
            model_mask_kwargs = {} 
        noise_schedule = NoiseScheduleVP(schedule="discrete", betas=torch.from_numpy(self.betas).to(device)) 
        ...
        dpm_solver = DPM_Solver( 
            ...
            correcting_x0_fn="dynamic_thresholding", #added to improve results 
        ) 
        t_start = 1.0 
        t_end = 1e-3 
        time_steps = dpm_solver.get_time_steps("logSNR", t_start, t_end, dpm_steps, device=device) 
        if progress: 
            from tqdm.auto import tqdm 
            time_steps_sliced = time_steps[:-1]  # Slice to exclude the last element 
            time_steps_sliced = tqdm(time_steps_sliced)  # Wrap with progress bar 
        else: 
            time_steps_sliced = time_steps[:-1]  # Just slice, no progress bar 
        for idx, t_continuous in enumerate(time_steps_sliced): 
            s_continuous = time_steps[idx + 1] 
            t_discrete = int((self.num_timesteps - 1) * (1 - t_continuous)) 
            t = torch.tensor([t_discrete] * shape[0], device=device) 
            with torch.no_grad(): 
                next_img = dpm_solver.singlestep_dpm_solver_update( 
                    img, 
                    s=t_continuous, 
                    t=s_continuous, 
                    order=3, 
                ...
                yield {"sample": out["sample"], "pred_xstart": out["pred_xstart"]} 
                img = out["sample"] 
    ...
    def ddim_sample(
        ...
        if pred_xstart is not None:
            gt_keep_mask = torch.zeros(x.shape).to(x.device)
            gt_keep_mask[model_mask_kwargs["ref_img"] > 0.] = 1
            gt_keep_mask[model_mask_kwargs["ref_img"] < 0.] = 0
